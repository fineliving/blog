...
function average(list) {
  ...
  var total = SIMD.Float32x4.extractLane(sum, 0) + SIMD.Float32x4.extractLane(sum, 1) + SIMD.Float32x4.extractLane(sum, 2) + SIMD.Float32x4.extractLane(sum, 3)
  return total / n
}
```
